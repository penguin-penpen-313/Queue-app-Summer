...
window.APP_CONFIG = {
  ...
  gift: {
    ...
    // ★何コインで花火1発か。10 なら 10C=1発（100C×8=800C → 80発）
    coinsPerShell: 10,

    maxShells: 1000,    // 発数の上限（10000C相当。これ以上のギフトも同じ扱い）
    minShells: 1,       // 最低発数
   ...
   *
   * ▼ 長さ
   *   maxDurationMs … 一番大きなギフト(=maxShells)のときの長さ。既定10秒
   *   発数がどれだけ多くても、この時間を超えないように詰めて打ちます。
   *
   * ▼ スペシャル花火
   *   発数が多いときは「10発ぶん＝スペシャル1発」に置き換えます。
   *   打ち上げ回数が1/10になるので、10秒に収まり、画面のごちゃごちゃも減ります。
   *   種類は「特大」と「ペンギン柄」（6種類のペンギンからランダム）。
   */
  fireworks: {
    enabled: true,

    minDurationMs:    900,    // 1発のときの長さ
    maxDurationMs:  10000,    // 最大発数のときの長さ（★ここで全体の長さを調整）
    maxGapMs:         700,    // 発射間隔の上限（少発のとき）
    minGapMs:          60,    // 発射間隔の下限（多発のとき）
    maxSimul:          14,    // 同時に上がる玉の数の上限（＝密度）

    maxRenderShells:  420,    // 通常玉の描画上限（負荷対策）
    maxParticles:    2400,    // 同時に存在する粒の上限（負荷対策）
    minShellParticles: 13,    // 1玉あたりの粒（多発時）
    maxShellParticles: 38,    // 1玉あたりの粒（少発時）
    maxShellScale:    2.4,    // 多発時の玉の大きさ倍率
    opacity:         0.78,    // 花火の濃さ（下げると順番待ちが読みやすい）
    centerClear:      0.7,    // 中央（順番待ちの表示）を避けて左右に上げる割合 0〜1

    /* スペシャル花火 */
    special: {
      enabled:      true,
      from:           30,   // 何発以上でスペシャルを混ぜ始めるか
      worth:          10,   // スペシャル1発 = 通常何発ぶんか
      share:        0.55,   // 全体の何割をスペシャルに置き換えるか（0〜0.9）
      max:            60,   // スペシャルの最大個数
      penguinRate:   0.5,   // スペシャルのうちペンギン柄の割合（残りは特大）
      // ペンギンの種類。減らすと出る種類が絞られる
      penguins: ['emperor', 'adelie', 'rockhopper', 'chinstrap', 'gentoo', 'african']
    }
  },
